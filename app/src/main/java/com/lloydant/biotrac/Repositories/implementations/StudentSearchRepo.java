...
public class StudentSearchRepo implements IStudentSearchRepository {
    ...
    @Override
    public Observable<Response<GetStudentsByDepartmentQuery.Data>> GetStudentsByDepartment(String department,
                                                                                           int level, String token) {
        ApolloCall<GetStudentsByDepartmentQuery.Data> apolloCall = ApolloConnector.setupApollo(token)
                .query(new GetStudentsByDepartmentQuery(department,level));
